...
public class XMIHandlerImpl implements XMIHandler {
	...
	private ArrayList dtdElementList = null;
	private ArrayList dtdAttList = null;
	...
	public MDSFile mapMDS2DTD(MDSModel mdsModel) throws XMIHandlerException {
		...
		try {
			...
			f =
				new BufferedReader(
					new FileReader(MDSGlobals.RESOURCES_PATH + "fixedContent.dtd"));
		...
		String intro = "\n<!-- ################################################### -->\n";
		intro += "<!-- # additional content generated by metadata.server # -->\n";
		intro += "<!-- ################################################### -->\n\n";
		i = classes.iterator();
		while (i.hasNext()) {
			dtdElementList = new ArrayList();
			dtdAttList = new ArrayList();
				...
				if (end1Class.getId().equals(mdsClass.getId())) {
					...
					if (end2.getAggregation()
						...
						if (mode.equals("all")) {
							...
							dtdAttList.add(end2Class.getLabel().toLowerCase());
							// rule 6e
	...
	private ArrayList getSubClasses(MDSClass superClass) {
		...
		while (i.hasNext()) {
			...
			if (generalization
				...
				dtdAttList.add(subClass.getLabel().toLowerCase());
