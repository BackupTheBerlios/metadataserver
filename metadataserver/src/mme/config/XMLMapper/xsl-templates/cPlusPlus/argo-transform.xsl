...
<xsl:template name = "QueryType" >
    ...
    <attribute_type>
        ...
        <xsl:with-param name="type" select = "/XMI/XMI.content/Model_Management.Model/Foundation.Core.Namespace.ownedElement/Model_Management.Package/Foundation.Core.Namespace.ownedElement/Model_Management.Package/Foundation.Core.Namespace.ownedElement/Foundation.Core.DataType[@xmi.id=$id]/Foundation.Core.ModelElement.name"/>
        </xsl:call-template>
    </attribute_type>
    <attribute_type_sql>
	...
	<extends>
		<xsl:for-each select="//Foundation.Core.Namespace.ownedElement">
			...
			<xsl:if test="contains($classId, $child)">
				<xsl:variable name="extends" select="Foundation.Core.Generalization.parent/Foundation.Core.GeneralizableElement/@xmi.idref"/>
				<generalclass Id="{Foundation.Core.Generalization.parent/Foundation.Core.GeneralizableElement/@xmi.idref}">
					<xsl:call-template name="extendName">
...
<xsl:template name="extendName">
<xsl:param name="name"/>
	<xsl:for-each select="//Foundation.Core.Class">
		<xsl:variable name="general" select="@xmi.id"/>
		<xsl:if test="contains($name, $general)">
			<xsl:value-of select="Foundation.Core.ModelElement.name/text()"/>
		</xsl:if>
	</xsl:for-each>
</xsl:template>
...
<xsl:template name="asso">
	<xsl:param name="classId"/>
    <xsl:for-each select="//Foundation.Core.Namespace.ownedElement/Foundation.Core.Association">
	<xsl:variable name="firstAssoId" select="Foundation.Core.Association.connection/Foundation.Core.AssociationEnd[position() = 1]/Foundation.Core.AssociationEnd.type/Foundation.Core.Classifier/@xmi.idref"/>
	<xsl:variable name="secondAssoId" select="Foundation.Core.Association.connection/Foundation.Core.AssociationEnd[position() = 2]/Foundation.Core.AssociationEnd.type/Foundation.Core.Classifier/@xmi.idref"/>
   		    ...
   		    <association>
   			    <xsl:for-each select="Foundation.Core.Association.connection/Foundation.Core.AssociationEnd">
	    	        <association_end>
	    	            ...
	    	            <association_multiplicy_lower><xsl:value-of select="Foundation.Core.AssociationEnd.multiplicity/Foundation.Data_Types.Multiplicity/Foundation.Data_Types.Multiplicity.range/Foundation.Data_Types.MultiplicityRange/Foundation.Data_Types.MultiplicityRange.lower"/></association_multiplicy_lower>
	    	            <association_multiplicy_upper><xsl:value-of select="Foundation.Core.AssociationEnd.multiplicity/Foundation.Data_Types.Multiplicity/Foundation.Data_Types.Multiplicity.range/Foundation.Data_Types.MultiplicityRange/Foundation.Data_Types.MultiplicityRange.upper"/></association_multiplicy_upper>
						<xsl:variable name="assoId" select="Foundation.Core.AssociationEnd.type/Foundation.Core.Classifier/@xmi.idref"/>
        	       		<association_type Id="{$assoId}">
            	    		<xsl:call-template name="extendName">
                				<xsl:with-param name="name" select="$assoId"/>
                			</xsl:call-template>
						</association_type>	                	
	        	    </association_end>	     
   	    		</xsl:for-each>
